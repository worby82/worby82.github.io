...
let searchBtn = document.querySelector('.btn.btn--search-form');
let noResult = document.querySelector('.title.title--no-result');
...
function makeCocktailCard(nameValue,shortDesciption,alcoholPresent,imageWebp,imageJpg,link) { //создание карточки
    ...
    cocktailImage.setAttribute('width', '100%');
    ...
    if(nameValue == 'Пусто') {
        ...
        cocktaiNoResultText.classList.add('text','text--no-result');
    ...
    cocktailName.classList.add('title','title--cocktail');
